...
async function main() {
  ...
  const headers = { apikey: anonKey, Authorization: `Bearer ${login.data.access_token}` }

  const moduleKey = 'ops'
  const cityCode = 'URB'

  const tests = [
    ['admin_upsert_feature_flag', { module_key: moduleKey, enabled: true, rollout_percent: 20, note: 'smoke rollout' }],
    ['admin_list_feature_flags', {}],
    ['admin_upsert_city_rollout', { city_code: cityCode, is_enabled: true, phase: 'pilot', note: 'smoke city rollout' }],
    ['admin_list_city_rollouts', {}],
    ['admin_rollout_rollback', { scope: 'city', city_code: cityCode, reason: 'smoke rollback city' }],
  ]

  for (const [fn, payload] of tests) {
    const res = await postJson(`${fnBase}/${fn}`, headers, payload)
    if (res.ok && res.data?.success === true) {
      pass(fn, `status=${res.status}`)
    } else {
...
main().catch((error) => {
  console.error('S7 rollout smoke error:', error)
  process.exit(1)
})
