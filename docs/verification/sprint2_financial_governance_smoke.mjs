...
async function main() {
  ...
  for (const [fn, payload] of checks) {
    ...
    const ok = res.ok && res.data?.success === true && Array.isArray(res.data?.data)
    if (ok) pass(fn, `status=${res.status}, items=${res.data.data.length}`)
    else {
...
main().catch((error) => {
  console.error('Sprint 2 smoke error:', error)
  process.exit(1)
})
